...
export class Neo4jService {
  ...
  async addNode(
    ...
    if (!properties.embedding) {
      ...
      properties.embedding =
        await this.openAIService.createEmbedding(textForEmbedding);
    }
    const cypher = `
  ...
  async updateNode(
    ...
    if (properties.content && !properties.embedding) {
      properties.embedding = await this.openAIService.createEmbedding(
        properties.content
      );
    }
    const cypher = `
  ...
  async connectNodes(
    ...
    const cypher = `
      MATCH (a:User {userId: $fromNodeId}), (b:User {userId: $toNodeId})
